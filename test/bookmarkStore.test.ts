...
describe('BookmarkStore', () => {
  ...
  it('collects every distinct tag across bookmarks, sorted alphabetically', () => {
    const store = newStore();
    store.addBookmark(
      makeBookmark({ id: 'a', uri: 'file:///repo/src/a.ts', tags: ['review', 'auth'] }),
    );
    store.addBookmark(
      makeBookmark({ id: 'b', uri: 'file:///repo/src/b.ts', tags: ['auth', 'todo'] }),
    );
...
describe('matchesSearchFilter', () => {
  const bookmark = makeBookmark({
    ...
    workspaceFolderName: 'backend-repo',
  });
