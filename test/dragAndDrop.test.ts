...
describe('sortByOrder', () => {
  ...
  it('falls back to the comparator when neither item has an order', () => {
    ...
    expect(sortByOrder(items, (a, b) => b.createdAt - a.createdAt).map((i) => i.id)).toEqual([
      'b',
      'a',
    ]);
  });
...
describe('dropBookmark', () => {
  ...
  it('reorders within the same folder scope in tree view without changing folderId', () => {
    ...
    dropBookmark(
      store,
      'b',
      new BookmarkTreeItem(makeBookmark({ id: 'a', folderId: folder.id }), []),
      'tree',
    );
  ...
  it('moves folders when dropped onto a bookmark in a different folder in tree view', () => {
    ...
    dropBookmark(
      store,
      'dragged',
      new BookmarkTreeItem(makeBookmark({ id: 'target', folderId: dest.id }), []),
      'tree',
    );
  ...
  it('reorders in place in list view without reassigning folderId', () => {
    ...
    dropBookmark(
      store,
      'root',
      new BookmarkTreeItem(makeBookmark({ id: 'grouped', folderId: folder.id }), []),
      'list',
    );
...
describe('BookmarksTreeProvider drag and drop', () => {
  ...
  it('handleDrop nests a folder end-to-end via the data transfer payload', async () => {
    ...
    dataTransfer.set(
      BOOKMARK_DND_MIME_TYPE,
      new DataTransferItem(JSON.stringify({ kind: 'folder', id: b.id })),
    );
  ...
  it('handleDrop reorders bookmarks end-to-end via the data transfer payload', async () => {
    ...
    dataTransfer.set(
      BOOKMARK_DND_MIME_TYPE,
      new DataTransferItem(JSON.stringify({ kind: 'bookmark', id: 'b' })),
    );

    await provider.handleDrop(
      new BookmarkTreeItem(makeBookmark({ id: 'a' }), []),
      dataTransfer as any,
    );
