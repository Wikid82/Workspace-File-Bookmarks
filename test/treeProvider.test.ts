...
describe('BookmarksTreeProvider', () => {
  ...
  describe('tag filter', () => {
    ...
    it('sets and reports the active tag filter, updating the context key', () => {
      ...
      expect(commands.executeCommand).toHaveBeenCalledWith(
        'setContext',
        'workspace-file-bookmarks.tagFilterActive',
        true,
      );
    });
  ...
  describe('search filter', () => {
    ...
    it('sets and reports the active search filter, updating the context key', () => {
      ...
      expect(commands.executeCommand).toHaveBeenCalledWith(
        'setContext',
        'workspace-file-bookmarks.searchFilterActive',
        true,
      );
    });
    ...
    it('narrows tree mode, including bookmarks nested in folders', () => {
      ...
      const folder = store.createFolder('Backend');
      store.addBookmark(
        makeBookmark({ id: 'match', folderId: folder.id, label: 'auth-service.ts' }),
      );
  ...
  describe('tree mode', () => {
    it('groups bookmarks under their folder, alphabetized, and lists ungrouped ones after', () => {
      ...
      expect(
        children
          .filter((c): c is FolderGroupItem => c instanceof FolderGroupItem)
          .map((f) => f.folder.name),
      ).toEqual(['Alpha', 'Zeta']);
      expect(
        children
          .filter((c): c is BookmarkTreeItem => c instanceof BookmarkTreeItem)
          .map((b) => b.bookmark.id),
      ).toEqual(['ungrouped-new', 'ungrouped-old']);
      void zFolder;
    });
    ...
    describe('nested folders', () => {
      ...
      it('lists child folders (alphabetized) before direct bookmarks when a folder is expanded', () => {
        ...
        expect(
          children.map((c) =>
            c instanceof FolderGroupItem ? c.folder.name : (c as BookmarkTreeItem).bookmark.id,
          ),
        ).toEqual(['Alpha Service', 'Zeta Service', 'direct']);
      });
      ...
      it('shows a subfolder count in the description alongside the bookmark count', () => {
        ...
        const node = new FolderGroupItem(
          parent,
          [makeBookmark({ folderId: parent.id })],
