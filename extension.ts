...
const folderFallbackCompare = (a: BookmarkFolder, b: BookmarkFolder) =>
...
export function eligibleParentFolders(
  store: BookmarkStore,
  options: { excludeFolderId?: string } = {},
): BookmarkFolder[] {
  const folders = store.getAllFolders();
  const subtreeHeight = options.excludeFolderId
    ? folderSubtreeHeight(folders, options.excludeFolderId)
    : 0;
  return folders.filter((f) => {
    if (
      options.excludeFolderId &&
      (f.id === options.excludeFolderId ||
        isDescendantFolder(folders, options.excludeFolderId, f.id))
    ) {
      return false;
    }
...
export function activate(context: vscode.ExtensionContext): ExtensionApi {
  ...
  vscode.commands.executeCommand(
    'setContext',
    TAG_FILTER_CONTEXT_KEY,
    provider.getTagFilter() !== null,
  );
  vscode.commands.executeCommand(
    'setContext',
    SEARCH_FILTER_CONTEXT_KEY,
    provider.getSearchFilter() !== null,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      (uri: vscode.Uri | undefined, uris: vscode.Uri[] | undefined) =>
        addBookmarksForUris(store, uri, uris),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.addBookmarkToFolder',
      (uri: vscode.Uri | undefined, uris: vscode.Uri[] | undefined) =>
        addBookmarksToFolder(store, uri, uris),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.removeBookmark',
      (item: BookmarkTreeItem) => store.removeBookmark(item.bookmark.id),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.renameBookmark',
      (item: BookmarkTreeItem) => renameBookmark(store, item),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.editTags', (item: BookmarkTreeItem) =>
      editBookmarkTags(store, item),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.filterByTag', () =>
      filterByTag(store, provider),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.clearTagFilter', () =>
      provider.setTagFilter(null),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.filterBookmarks', () =>
      filterBookmarks(provider),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.clearSearchFilter', () =>
      provider.setSearchFilter(null),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.openBookmark', (bookmark: Bookmark) =>
      openBookmark(bookmark),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.createFolder', () =>
      createFolder(store),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.createSubfolder',
      (item: FolderGroupItem) => createSubfolder(store, item),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.moveFolderToParent',
      (item: FolderGroupItem) => moveFolderToParent(store, item),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.renameFolder',
      (item: FolderGroupItem) => renameFolder(store, item),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.deleteFolder',
      (item: FolderGroupItem) => deleteFolder(store, item),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.moveToFolder',
      (item: BookmarkTreeItem) => moveToFolder(store, item),
    ),
    vscode.commands.registerCommand(
      'workspace-file-bookmarks.openAllInFolder',
      (item: FolderGroupItem) => openAllInFolder(store, item),
    ),
    vscode.commands.registerCommand('workspace-file-bookmarks.setViewModeList', () =>
  ...
  implements vscode.TreeDataProvider<TreeNode>, vscode.TreeDragAndDropController<TreeNode>
{
  ...
  getChildren(element?: TreeNode): TreeNode[] {
      ...
      return sortByOrder(bookmarks, bookmarkFallbackCompare).map(
        (b) =>
          new BookmarkTreeItem(
            b,
            describeBookmark(b, isMultiRoot, folderBreadcrumb(folders, b.folderId ?? '') || null),
          ),
      );
    }
    ...
    if (element instanceof FolderGroupItem) {
      const childFolderNodes = sortByOrder(element.childFolders, folderFallbackCompare).map(
        buildFolderNode,
      );
  ...
  handleDrag(source: readonly TreeNode[], dataTransfer: vscode.DataTransfer): void {
    ...
    const payload =
      item instanceof BookmarkTreeItem
  ...
  async handleDrop(target: TreeNode | undefined, dataTransfer: vscode.DataTransfer): Promise<void> {
    ...
    const payload = JSON.parse(await transferItem.asString()) as {
...
export async function createSubfolder(store: BookmarkStore, item: FolderGroupItem) {
  if (!canNestUnder(store.getAllFolders(), item.folder.id)) {
    vscode.window.showWarningMessage(
      `Can't create a subfolder here — maximum folder depth (${MAX_FOLDER_DEPTH}) reached.`,
    );
    return;
  }
  const name = await vscode.window.showInputBox({
...
export async function moveFolderToParent(store: BookmarkStore, item: FolderGroupItem) {
  const parentId = await pickParentFolder(
    store,
    `Move "${item.folder.name}" to...`,
    item.folder.id,
  );
...
export async function renameBookmark(store: BookmarkStore, item: BookmarkTreeItem) {
  const label = await vscode.window.showInputBox({
    ...
    validateInput: (value) =>
      value.trim().length === 0 ? 'Bookmark label cannot be empty.' : undefined,
  });
  if (label) {
...
export async function editBookmarkTags(
  store: BookmarkStore,
  ...
  if (picked.some((p) => p.label === ADD_NEW_TAG_PICK)) {
    const newTag = await vscode.window.showInputBox({
      prompt: 'New tag name',
      validateInput: (value) =>
        value.trim().length === 0 ? 'Tag name cannot be empty.' : undefined,
    });
    if (newTag) {
...
export async function pickParentFolder(
  ...
  const picked = await vscode.window.showQuickPick(
    [NO_PARENT_PICK, ...eligible.map((f) => breadcrumbById.get(f.id) as string)],
    {
      placeHolder,
    },
  );
...
export function dropFolder(store: BookmarkStore, draggedId: string, target: TreeNode | undefined) {
  ...
  if (target instanceof FolderGroupItem) {
    if (
      target.folder.id === draggedId ||
      isDescendantFolder(folders, draggedId, target.folder.id)
    ) {
      return;
    }
    ...
    if (!canNestUnder(folders, target.folder.id, height)) {
      vscode.window.showWarningMessage(
        `Can't nest "${dragged.name}" there — maximum folder depth (${MAX_FOLDER_DEPTH}) reached.`,
      );
      return;
    }
...
export function dropBookmark(
  ...
  const destinationFolderId =
    viewMode === 'list'
      ? dragged.folderId
      : target instanceof BookmarkTreeItem
        ? target.bookmark.folderId
        : null;
  ...
  const scope = store
    .getAllBookmarks()
    .filter((b) => (viewMode === 'list' ? true : b.folderId === destinationFolderId));
...
export async function addBookmarksToFolder(
  ...
  const placeHolder =
    targets.length > 1
      ? `Add ${targets.length} files to...`
      : `Add "${toBookmark(targets[0]).label}" to...`;
